...
########################################################
# 해야할 것) 아웃된 플레이어를 제외하고 턴넘김 고려.
# 의문사항) 현재: 지목한 상대의 패를 2라 유츄 > 상대방이 [2,4] 중 4를 선택 > 4가 되고 그냥 넘어감
#  ㄴ지목 당한 상대가 4로 선택시 맞춤에 실패 하였다고 판단하고 먹은 타일을 공개해야 하는 것이 아닌지.
#  ㄴ현재 벨런스상 스포키 넘버 두개중에 한개도 맞추기 힘든데 이거마저 추가하면 맞추는 쪽이 너무 불리함
#### ####################################################
...
def next_turn(i):
    ...
    if turn == pn+1:
        turn = 1
    win = 0
    while 1: # 공개필드의 ?개수를 세서 다음플레이어의 ? 수가 0개면(즉 다 오픈됨) 다음 플레이어에게 차례가 넘어감
        if count_qm(public_field[turn-1]) == 0:
            turn += 1 # 근데 나 빼고 모두 ? 수가 0개여서 다시 턴이 나한테 오면 승리하도록 만듬
            win += 1
            if turn == pn+1: 
                turn = 1
        else:
    ...
    while 1:
        if len(ti_b) == 0 and len(ti_w) == 0:
            ...
            if choice_color == "b" and len(ti_b) != 0:                      # 입력받은 색깔이 b,w에 따라 그에 맞는 색카드를
                ...
                if recent_card == [1,[100,100]]:
                    zoker[turn-1] = [1,[100,100]]
                ...
                if zoker[turn-1][1][0] == 100: # 게임 중간에 조커를 뽑을 수도 있으니 조커 구문을 추가
                    while 1:
                        print("플레이어",turn,"은 조커 놓을 자리를 입력해주세요 입력하신 숫자 이전칸에 조커가 들어갑니다")
                        print("만약 기존 조커자리를 입력하시면 조커의 위치는 변하지 않습니다")
                        print("플레이어",turn,"의 패:",p[turn-1])
                        zoker_arrange = float(input("조커 놓을 위치를 입력해주세요"))
                        zoker_arrange = int(zoker_arrange)
                        if zoker_arrange >= 1 and zoker_arrange <= len(p[turn-1]):
                            p[turn-1][p[turn-1].index(zoker[turn-1])] = 0
                            p[turn-1].insert(zoker_arrange-1,zoker[turn-1])
                            p[turn-1].remove(0)
                            zoker[turn-1] = [0,[0]]
                            break
                        else:
                ...
                if recent_card == [0,[100,100]]:
                    zoker[turn-1] = [0,[100,100]]
                p[turn-1].append(recent_card)
                public_field[turn-1].append([0,"?"])
                ti.pop(ti.index(recent_card))   # 뽑은 카드를 다시 뽑으면 안되니 뽑은 카드는 필드에서 삭제함
                arrange(turn-1)          # 뽑았으니 재배열함
                if zoker[turn-1][1][0] == 100: # 조커도 흑,백색으로 나누어 져있어서 두번 들어감
                    while 1:
                        print("플레이어",turn,"은 조커 놓을 자리를 입력해주세요 입력하신 숫자 이전칸에 조커가 들어갑니다")
                        print("만약 기존 조커자리를 입력하시면 조커의 위치는 변하지 않습니다")
                        print("플레이어",turn,"의 패:",p[turn-1])
...
def c_card():            
    global choice_card, one_more, bulldozer_count, bulldozer_num
    while 1:                        # 지목한 플레이어의 카드를 지목하고 유추한 후 상황을 전개하는 코드
        ...
        if choice_card > len(p[choice_player-1]) or choice_card < 1:
            ...
            if player_ablilty[turn-1][0] == 1: # 패시브 능력이 1번이면(평균을 보여줌) 이 구문에 들어옴
                ability.average_ability(p,choice_player,choice_card)
            elif player_ablilty[turn-1][0] == 2: # 패시브 능력이 2번이면 (스포키 차이를 보여줌) 이 구문에 들어옴
                ability.diffrence_ability(p,choice_player,choice_card)
            print("플레이어",choice_player,"의 패:",public_field[choice_player-1])
            print("나의 패:",p[turn-1])
            if player_ablilty[turn-1][1] == 2: # 액티브 능력이 2번이면 (불도저) 이 구문에 들어감
                ability.bulldozer(bulldozer_count,pn)
            choice_num = float(input("그 카드의 숫자는 무엇입니까:"))       # 제대로 골랐으니 숫자를 찍어라 함 
            choice_num = int(choice_num)                                  # 유추한 다음 룰에 맞게 상황을 조정
            before_choice_card[turn-1][choice_player-1] = p[choice_player-1][choice_card-1] # 이전에 무슨 카드를 골랐는지 알려주기 위함
            if choice_num != p[choice_player-1][choice_card-1][1][0] and choice_num != p[choice_player-1][choice_card-1][1][1]:
                ...
                print("그런 숫자가 있습니다 룰에 따라 지목받은 플레이어는 카드를 붕괴합니다") 
                if bulldozer_num == 1: # 불도저 능력을 사용해서 맞추면 불도저 붕괴가 일어남
                    ability.bulldozer_collapse(choice_player,choice_card,p)
                elif bulldozer_num == 0:                                                  # 스포키 숫자 한개라도 맞춰서
...
def select_ability():
    global player_ablilty_backup
    print("각 플레이어별 능력이 랜덤으로 주어집니다 능력은 중복될 수 있습니다")
    for i in range(0,pn):
        player_ablilty[i][0] = random.choice(p_ablilty_index)   
        player_ablilty[i][1] = random.choice(a_ablilty_index)             
    player_ablilty_backup = copy.deepcopy(player_ablilty)   
################################################################################# 게임 진행
...
bulldozer_count = 0 # 불도저 능력사용을 위한 변수
bulldozer_num = 0
...
p_ablilty_index = list(range(1,3)) # 패시브 능력 리스트
a_ablilty_index = list(range(1,3)) # 엑티브 능력 리스트
player_ablilty = list(numpy.zeros(pn)) # 첫번째 능력은 패시브 두번째 능력은 엑티브다
player_ablilty_backup = list(numpy.zeros(pn))
p = list(range(0,pn))   # 플레이어 묶음 생성
public_field = list(range(0,pn))  # 공개된 카드 필드
before_choice_card = list(range(0,pn)) # 이전에 어떤 카드를 골랐는지 편의성 제공을 위해서 만듬
zoker = list(range(0,pn))
for i in range(0,pn):
    before_choice_card[i] = list(range(0,pn))
    zoker[i] = [0,[0]]
    player_ablilty[i] = [0,0]
...
for i in list(range(0,pn)): # 플레이어 전체에 대해 스포키 수 배열    
    ...
    if zoker[i][1][0] == 100: # 조커를 뽑으면 배열하도록 코딩
        while 1:
            print("플레이어",i+1,"은 조커 놓을 자리를 입력해주세요 입력하신 숫자 이전칸에 조커가 들어갑니다")
            print("만약 기존 조커자리를 입력하시면 조커의 위치는 변하지 않습니다")
            print("플레이어",i+1,"의 패:",p[i])
            zoker_arrange = float(input("조커 놓을 위치를 입력해주세요"))
            zoker_arrange = int(zoker_arrange)
            if zoker_arrange >= 1 and zoker_arrange <= len(p[i]):
                p[i][p[i].index(zoker[i])] = 0
                p[i].insert(zoker_arrange-1,zoker[i])
                p[i].remove(0)
                zoker[i] = [0,[0]]
                break
            else:
...
while 1: # 능력모드 사용 유무 알아보기
    rty = input("능력모드를 사용하시겠습니까? (y/n)")
    if rty == "y":
        print("능력 모드로 진행합니다.")
        select_ability()
        break
...
while 1:
    ...
    while 1:
        one_more = 0 # 연속해서 맞추려고 할때 필요한 변수
        c_p() # 플레이어 지목
        c_card() # 카드 숫자 유추
        ...
        if one_more == 0:
            break
    if player_ablilty[turn-1][1] == 1: # 플레이어 능력을 보고 그게 1번 엑티브 능력(사일런스)면 이 구문에 들어옴
        ability.silence(pn,public_field,turn,player_ablilty)
    if player_ablilty[turn-1] == [-1,-1]: # 사일러스의 침묵을 맞은 뒤 풀려나게 하기 위한 코딩(-1 이란건 침묵에 걸렸단 거임) 
        player_ablilty[turn-1][0] = player_ablilty_backup[turn-1][0]
        player_ablilty[turn-1][1] = player_ablilty_backup[turn-1][1]
    next_turn(turn) # 턴바꿈
