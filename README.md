...
- TO-DO) >> 진행 중..
    > 용; 서버관련 작업 & 승리시 페이지 구현 및 승리 조건.
    > 세형; 오픈된 타일 클릭 막기(오픈 여부 나누는 선행작업 필요). & 이론 및 난의도 팝업창 정리.
    > 지원; (연쇄 붕괴) 얽힌 타일 붕괴.
...
세형;
- 이론창에 대한 틀 Tk로 생성, 이미지 크기 조절가능하게 함 (PIL 모듈이 추가로 필요해짐)
    > option room에서 추가되었고 용이가 이론 부분을 도움말로 옮김
