...
- 배경음악 밎 버튼 사운드 활성화.
    > 버튼 함수 경량화 및 수정. (Music & Sound 구별 주의할 것. 아니면 버튼 클릭시 배경음 꺼짐.)
    
- TO-DO) >> 진행 중..
    > 용; 서버관련 작업. & 이론 팝업창 정리.
    > 세형; 승리시 페이지 구현 및 승리 조건연결. & 난이도 팝업창 구현 및 정리.
        > 난이도 체크 정보 txt 저장 및 불러오기 구현.
            (예상 체크 사항: 색상정렬 규칙, 상대 확률 보기, 아이템 사용 on/off)
    > 지원; 오픈된 타일 클릭 막기(오픈 여부 나누는 선행작업 필요).
