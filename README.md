...
- 지목(플레이어를 지목 x. 바로 특정 플레이어의 카드 지목)시 추측 함수 생성 및 연결 완.
...
- 4. 지목(플레이어를 지목 x. 바로 특정 플레이어의 카드 지목)시 추측 함수 생성 및 연결. >> 완.
    > 1차. 해당 값에 타일의 spooky 수 유무 검토 있으면 해당 타일 붕괴. >> 완.
    > 2차. (연쇄 붕괴) 위의 타일의 붕괴에 따라 알고리즘 쪽 함수 가져와서 얽힌 타일 붕괴. >> 진행 중..(지원)
    > 2차. 유추 실패시 먹은 타일 붕괴후 지원이 함수에 연결 >> 진행 중..(세형)
