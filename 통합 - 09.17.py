...
#   1) 타일에 색상 정보 추가 및 섞기======================================
tb = list(range(0,max_card_num+1))
tw = list(range(0,max_card_num+1))
...
#   2) 플레이어 수와 시작 타일 수지정 및 배포=============================
pn = 2                  #플레이어 넘버: 총 플레이어 수
p = list(range(0,pn))
stn = 3                 #스타팅 타일 넘버: 시작시 받는 타일 개수 (3개 가정)
...
for i in list(range(0,pn)):
    ...
    for k in list(range(0,stn)):
        if dummy[k][1][0] < dummy[k][1][1]: # 두 spooky 수 비교 작으면 놔둠
            pass
        else:
...
#   4) 평균값 정렬========================================================
for q in range(0,stn):                      #완전히 정렬 위해 반복 횟수 stn 수로 설정
    for i in list(range(0,pn)):
        dummy = p[i]
