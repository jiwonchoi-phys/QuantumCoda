...
max_card_num = 5
card_num_kind = list(range(0,max_card_num+1)) #0부터 max_card_num까지 리스트 생성
card_num = []    #양자 번호
for i in range(0,max_card_num+1):
    card_num.append(card_num_kind[i])
    card_num.append(card_num_kind[i])   #두번 넣어서 spooky수 생성
random.shuffle(card_num)                #섞는다
...
#print("field_count",field_count) #확인후 주석처리
#print("숫자:",field_num) #확인후 주석처리
print("루프:",loop_check) #블랙만 루프 확인 되는 건가??
...
for i in range(0,pn): # 플레이어 수만큼 리스트 생성
    p[i] = []
    public_field[i] = []
    for k in range(0,stn): #플레이어 리스트에 타일 배분 -> 배분 과정이 설정 스타팅 타일 개수에 의존
        qwer = random.choice(ti) # 랜덤을 뽑는데 사실 위에서 ti 리스트가 이미 랜덤 셔플으로 섞여있는 상태입니다
        p[i].append(qwer)
        ti.pop(ti.index(qwer))
        if qwer[0] == 0:
            public_field[i].append([0,"?"]) # 공개 필드에 알수없는(?)카드를 추가 [0 흰색, 1 검정]
        elif qwer[0] == 1:
...
def action(turn):        # turn인 플레이어가 취하는 액션                                                       
    ...
    for i in range(0,pn):
        for k in range(0,len(public_field[i])):
            if public_field[i][k][1] != "?" and public_field[i][k] != p[i][k]:
                public_field[i][p[i].index(public_field[i][k])] = p[i][p[i].index(public_field[i][k])]
                public_field[i][k][0] = public_field[i][p[i].index(public_field[i][k])][0]
                public_field[i][k][1] = "?"
